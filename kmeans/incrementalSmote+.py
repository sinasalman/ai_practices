...
def incremental_kmeans(X, k):
    ...
    k = min(15, k)  # Limit k to optimize
    c = [np.mean(X, axis=0)]
    t = 1
...
def incremental_smote(X, y, irt=2.0):
    ...
    # Convert Y to numpy array before filtering
    Y = np.array(Y)
    
    # Filter with KNN (Idea 4)
    if len(Y) > 0:  # Check if Y is not empty
        knn = KNeighborsClassifier(n_neighbors=5)
        knn.fit(X, y)
        y_pred = knn.predict(Y)
        if len(y_pred) != len(Y):
            raise ValueError(f"Length mismatch: y_pred ({len(y_pred)}) vs Y ({len(Y)})")
        Y = Y[y_pred == 0]  # Keep only points predicted as minority
    else:
        Y = np.array([])  # Return empty array if no synthetic points
